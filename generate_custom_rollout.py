#!/usr/bin/env python3
"""Generate a deterministic custom-analysis rollout bundle from a checkpoint."""
...
from custom_benchmark_specs import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--render-episodes-every", action="store_true", help="Render and save every benchmark episode.")
...
def main() -> None:
    ...
    joint_velocities = []
    foot_slip_speed = []
    render_requests: list[tuple[int, str, list[Any]]] = []
    ...
    np.savez(
        ...
        episode_label=np.asarray(episode_labels),
    ...
    for episode_idx, label, trajectory in render_requests:
        video_path = output_dir / f"custom_eval_episode{episode_idx:02d}_{label}.mp4"
        frames = env.render(
            trajectory,
